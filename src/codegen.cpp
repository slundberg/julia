...
static void emit_assignment(jl_value_t *l, jl_value_t *r, jl_codectx_t *ctx)
{
    ...
    if (vi.memloc) {
        ...
        if (jl_is_symbol(r) && (!rval_info.typ || rval_info.typ == jl_bottom_type)) {
            if (vi.usedUndef) {
                // sometimes x = y::Union{} occurs
                jl_sym_t *s = (jl_sym_t*)r;
    ...
    if (ctx.debug_enabled) {
        ...
        if (va && ctx.vaSlot != -1) {
            ...
            ctx.slots[ctx.vaSlot].dinfo = ctx.dbuilder->createParameterVariable(
                SP,                     // Scope (current function will be fill in later)
                jl_symbol_name(slot_symbol(ctx.vaSlot, &ctx)),        // Variable name
